...
public class ClientConfig {
    ...
    private static final ForgeConfigSpec.ConfigValue<List<? extends String>> KEY_CATEGORIES;
    private static final ForgeConfigSpec.ConfigValue<List<? extends String>> CATEGORY_ORDER;
    private static final ForgeConfigSpec.ConfigValue<List<? extends String>> HIDE_KEYS;
    ...
    public static Map<KeyMapping, String> getKeysToChange() {
        ...
        for (String s : KEY_CATEGORIES.get()) {
            ...
            if (array.length != 2) {
                Keylist.LOGGER.warn("Could not process config value " + s + ": format is invalid");
                continue;
            }
        ...
        for (String s : CATEGORY_ORDER.get()) {
            ...
            if (array.length == 2 && !array[1].isEmpty()) {
                try {
                    ...
                    Keylist.LOGGER.warn("Could not process config value " + s + ": value \"" + array[1] + "\" is not a number");
                }
            } else {
                Keylist.LOGGER.warn("Could not process config value " + s + ": format is invalid");
            }
