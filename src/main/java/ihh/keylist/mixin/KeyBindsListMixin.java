...
public class KeyBindsListMixin {
    ...
    private Object[] filterKeysToHide(Object[] array, Operation<KeyMapping[]> original) {
        List<String> hide = ClientConfig.getKeysToHide();
        return Arrays.stream(array).filter(e -> !hide.contains(((KeyMapping) e).getName())).map(e -> (KeyMapping) e).toArray(KeyMapping[]::new);
    }
}
